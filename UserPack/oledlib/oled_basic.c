...
void UpdateScreen_PART(int x0,int y0,int x1,int y1)
{
	UpdateScreenDisplay_PART( x0, y0, x1, y1);
}
///////////////////////////////////////////////////////////////////////////////////////////////
//给选择的缓冲区写入8位数据
...
{
	int i,j;
	int temp =(y+height-1)/8-y/8;	//需要填充的矩形在屏幕中所占的行数 屏幕被分为8或16大行
	if(x>SCREEN_COLUMN ||y>SCREEN_ROW)  //超出范围 
			return;
